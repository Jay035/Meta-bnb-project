...
export default function Navbar() {
  ...
  return (
    <header
      data-aos="fade-in"
      className="flex items-center justify-between gap-14 px-6 py-6 w-full mx-auto max-w-[1240px] lg:gap-0 lg:py-[2.7rem] xl:px-0 "
    >
      <img
        data-aos="fade-right"
        className="w-44 lg:w-60 cursor-pointer"
        src={logo}
        alt="logo"
        onClick={() => navigate("/")}
      />
      ...
      <nav
        className={`
          ${
            menuState
              ? `absolute top-0 left-0 bg-black/75 w-full flex flex-col gap-12 lg:pl-0 text-xl lg:items-center lg:bg-transparent lg:w-fit lg:flex-row`
              : `hidden lg:flex `
          } lg:relative `}
      >
        {/* Menu */}
        <div className="flex flex-col gap-8 pt-8 pl-6 bg-white h-screen w-3/4 lg:pt-0 lg:pl-0 lg:w-fit lg:h-fit lg:flex-row lg:items-center">
          ...
          <ul className="flex flex-col gap-6 md:text-xl lg:flex-row lg:gap-12">
            <NavLink
              to="/"
              className="w-fit cursor-pointer hover:font-medium transition-all"
              onClick={() => setMenuState((prevState) => !prevState)}
            >
              Home
            </NavLink>
